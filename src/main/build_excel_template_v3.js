...
export const buildTemplate = (outputPath, sheetName, sheetTitle) => {
  let workbook = new Excel.Workbook();

  return new Promise((resolve, reject) => {
    if (!fs.existsSync(outputPath)) {
      return resolve(writeTemplate(outputPath, workbook, sheetName, sheetTitle));
    } else {
...
function writeTemplate(outputPath, workbook, sheetName, sheetTitle) {
  return new Promise((resolve, reject) => {
    let worksheet = workbook.addWorksheet(sheetName, {});
    writeBaseTemplate(workbook, worksheet, sheetTitle);
    // Write to File
...
function writeBaseTemplate(workbook, worksheet, sheetTitle) {
  ...
  worksheet.getCell('E1').value = sheetTitle;
